...
class MemberApiController extends Controller
{
    ...
    public function mpesaPush(Request $request)
    {
        ...
        $phone = $this->mpesa->normalizePhone($request->phone ?? $user->phone);

        if (!$phone) {
            return response()->json(['message' => 'Invalid phone number. Use a valid Safaricom/Airtel number, e.g. 0712345678.'], 422);
        }
